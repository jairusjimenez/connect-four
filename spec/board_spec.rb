...
describe 'Board' do
	...
	describe "#down_winner?" do
		...
		it "returns true when a winner occurs along a column with different type of tokens" do
			@board.place_token("\u{2605}", 2)
			4.times do
				@board.place_token("\u{2605}", 2)
			end
	...
	describe "#left_winner" do
		...
		it "returns false when a row contains less than 4 tokens" do
			3.downto(1) { |x| @board.place_token("\u{2605}", x) }
			expect(@board.left_winner?("\u{2605}", 2)).to be(false) 
		end

		it "returns false when tere are four tokens along a row of differing marks" do
			3.downto(1) { |x| @board.place_token("\u{2605}", x) }
			@board.place_token("\u{2605}", 0)
			expect(@board.left_winner?("\u{2605}", 2)).to be(false) 
		end

		it "returns true when there are four tokens along a row of the same mark" do
			3.downto(0) { |x| @board.place_token("\u{2605}", x) }
	...
	describe "#right_winner" do
		...
		it "returns false when a row contains less than 4 tokens", :right => true do
			2.upto(4) { |x| @board.place_token("\u{2605}", x) }
			expect(@board.right_winner?("\u{2605}", 2)).to be(false) 
		end

		it "returns false when tere are four tokens along a row of differing marks", :right => true do
			2.upto(4) { |x| @board.place_token("\u{2605}", x) }
		...
		it "returns true when there are four tokens along a row of the same mark", :right => true do
			2.upto(5) { |x| @board.place_token("\u{2605}", x) }
	...
	describe "#down_right_winner?" do
		...
		it "returns true when tere are four tokens along starting in a high row", :downright => true do
